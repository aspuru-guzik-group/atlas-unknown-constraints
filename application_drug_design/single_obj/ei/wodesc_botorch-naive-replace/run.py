...
feas_strategy = 'naive-replace'
feas_param = 0.0
with_descriptors = False
descriptors_type = None  # 'mord' or 'pca'
type_ = 'categorical'
...
def set_param_space(type_='categorical'):
	...
	if type_ == 'continuous':
		...
		if with_descriptors:
			...
			# scramble order of options if there are no descriptors
...
for num_repeat in range(missing_repeats):
	...
	data = pd.DataFrame({'template': template_col, 'alkyne': alkyne_col, 'abl1_pIC50': abl1_pIC50_col})
